<template>
    ...
    <div class="nav">
        ...
        <div class="content">
            <div class="header clear"><h2><img src="item.auth_icon" alt=""/></h2><p>{{detail.title}}</p></div>
            <div class="cont">
                <h3>{{detail.author}}</h3>
                <div class="time"><p>{{detail.time}}<span><img src="../assets/img/zan.png" alt=""/></span></p></div>
                <div class="text-box">{{detail.content}}</div>
            </div>
        </div>
        <div class="foot-btn">
        <ul>
 ...
 <style scoped>
    ...
    .content .header img{width:0.3rem;height: 0.3rem;border-radius: 50%}
  ...
  export default {
      ...
      mounted(){
          ...
          axios({
              ...
              //res.data是一个数组，需要转成对象
          )
          this.$root.$data.headerdisplay = false;
          this.$root.$data.footerdisplay = false;
